...
uint32_t Filter_GetNDelXAvg(uint8_t cmd , uint16_t GetN , uint16_t DelX)
{
	...
	for ( n = 0 ; n < total ; n++)
	{
		...
		{
			data_sample[n]= (uint32_t)readHumidity(AHTXX_FORCE_READ_DATA);		
		}
		else if (cmd == read_Temperature)
		{
			data_sample[n]= (uint32_t)readTemperature(AHTXX_FORCE_READ_DATA);
		}				
	}
...
{
	if (is_flag_set(flag_AHT10_GetData))
	{
		set_flag(flag_AHT10_GetData , DISABLE);
		
		#if 1	// display float

		printf("Humidity:%4.6f,Temperature:%4.6f\r\n" , 
		readHumidity(AHTXX_FORCE_READ_DATA) , readTemperature(AHTXX_FORCE_READ_DATA));

		#else	// display unsigned int
		convert_Humidity_data = Filter_GetNDelXAvg(read_Humidity , NUM_AVG_TRAGET , NUM_SAMPLE_DELETE);
		convert_Temperature_data = Filter_GetNDelXAvg(read_Temperature , NUM_AVG_TRAGET , NUM_SAMPLE_DELETE);

		printf("Humidity:%6d,Temperature:%6d\r\n" , 
		convert_Humidity_data , convert_Temperature_data);
		#endif	
	}
}
...
//        	printf("%s : %4d\r\n",__FUNCTION__,LOG++);
		...
		if ((get_tick() % 2000) == 0)
		{
			set_flag(flag_AHT10_GetData , ENABLE);
		}		
    }
}
